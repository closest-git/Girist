...
int CVM_head_off( CVM_SOLVER *hGAC )	{
	return hGAC->ldu+1;
}
*/
...
/*
	...
	v0.1	cys
		3/27/2008	
*/
void CVM_set_boundary( CVM_SOLVER *hGAC )	{
...
void CVM_smooth_u_( CVM_SOLVER *hGAC )	{
...
//fast marching method/*	
	for( r = 0; r < M+2; r++ )	{	//initialization fast marching method	
		for( c = 0; c < N+2; c++ )	{
			pos = CVM_POS(r,c,ldu);	
			if(	BIT_TEST(tag[pos],GIP_CVM_INTERFACE) )			{	
				BIT_SET(tag[pos],GIP_FMM_KNOWN);	
				for( i = GIP_NEIBOR_LEFT; i <= GIP_NEIBOR_UP; i++ )	{
					if( BIT_TEST(tag[pos],border_flag[i]) )
						continue;
					pos_1 = pos + neibor[i];
					if( BIT_TEST(tag[pos_1],GIP_CVM_INTERFACE) || BIT_TEST(tag[pos_1],GIP_FMM_BAND) )	
						continue;
...
//verify |delta U|=1.0
	...
	if( 1 )	{
		...
		average /=(M*N);
		printf( "Average |detaU|=%g\r\n",average );
	}
	if( 0 )	{
		char sTitle[80];
		sprintf( sTitle,"u_%d",++g_img_no );
...
/*
	...
	v0.1	cys
		4/23/2008	
*/
double CVM_norm_Hu_( int M,int N,double *u,int ldu,double *d )	{
	...
	printf( "%d:	|detaU|	Band Average=%g,Average=%g,max=%g,min=%g,kink=%g\r\n"
...
//	CVM_norm_Hu_( M,N,hGAC->u_buffer+hGAC->shift,hGAC->ldu );		//verify |delta U|=1.0
}
...
/*
	...
	v0.1	cys
		4/2/2008	
*/
void CVM_Init_u0( CVM_SOLVER *hGAC,GIP_IMAGE *hMask,int type )	{
	...
	if( hMask==0x0 )	{
		for( r = 0; r < M+2; r++ )	{
			for( c = 0; c < N+2; c++ )	{
				...
				if( u_buffer[pos]==0.0 )			{	
					BIT_SET( hGAC->tag[pos],GIP_CVM_INTERFACE );	
				}
			}
		}
...
/*
	...
	v0.2	cys
		5/6/2008	
*/
void CVM_ReInit_u0( CVM_SOLVER *hGAC,int type )	{
...
/*
	...
	v0.1	cys
		3/19/2008	
*/
void CVM_init( GIP_IMAGE *hIMG,GIP_IMAGE *hIMG_mask,CVM_SOLVER *hGAC )	{
	int M,N,MN,r,c,pos;
	...
	memset( hGAC,0x0,sizeof(CVM_SOLVER) );
	...
	for( r = 1; r < M-1; r++ )	{
	for( c = 1; c < N-1; c++ )	{
		pos=CVM_POS( r,c,N );		//需被替换为CVM_POS
		a = g[pos];
...
//	GIP_show_IplImage_d( M,N,"gy",hGAC->gy,N,GIP_OPENCV_CREATEWND );
	...
	CVM_Init_u0( hGAC,hIMG_mask,0x0 );
	CVM_smooth_u_( hGAC );
GIP_exit:	
	GIP_free( val );
}
...
/*
	...
	v0.1	cys
		3/26/2008	
*/
void CVM_clear( CVM_SOLVER *hGAC )	{
...
/*
	...
	v0.1	cys
		3/19/2008	
*/
void CVM_step( CVM_SOLVER *hGAC,double dT,double *temp )	{
	...
	for( r = 0; r < M; r++ )	{
		...
		for( c = 0; c < N; c++ )	{
			pos=CVM_POS(r+1,c+1,hGAC->ldu);			ASSERT( pos==u-hGAC->u_buffer );
			g_pos=CVM_POS(r,c,N);	
...
//		u = hGAC->u_buffer+hGAC->shift+r*ldu;
		for( c = 0; c < N; c++ )	{
			pos=CVM_POS(r+1,c+1,hGAC->ldu);			
			u[pos] += temp[pos];
		}
	}

	printf( "\tK_max=%g,K_min=%g\r\n",K_max,K_min );
...
void CVM_test( GIP_IMAGE *hIMG,GIP_IMAGE *hIMG_mask,char* sWndName )	{
	...
	double *temp;
	CVM_SOLVER gac,*hGAC=&gac;
	int M,N,t_len,isTrace=1,nKey,t_step=1;
	char sTitle[80];

	CVM_init( hIMG,hIMG_mask,&gac );
...
//			GIP_save_IplImage_d( M,N,sTitle,(temp+gac.shift),gac.ldu,0x0 );
		...
		average = CVM_norm_Hu_( M,N,hGAC->u_buffer+hGAC->shift,hGAC->ldu,temp+hGAC->shift );		
		sprintf( sTitle,".\\trace\\d_%d.bmp",gac_step );		
		GIP_save_IplImage_d( M,N,sTitle,(temp+gac.shift),gac.ldu,0x0 );
		CVM_step( &gac,dT,temp );
 		if( BIT_TEST(gac.flag,CVM_SOLVER_CONVERGE) )
			break;
		gac_step++;
		//if( average > 2 )	{
			CVM_ReInit_u0( &gac,0x0 );
			CVM_smooth_u_( &gac );
		//}
		t += dT;
	}
