...
/*
	...
	v0.1	cys
		1/19/2009	
*/
G_FILE_TYPE _File_Type( TCHAR *ext )		{
	G_FILE_TYPE type;
...
_FIO_EXIT_:
	...
	{	ret=-7;			}
	if( ret==0 )	{
		G_PRINTF( _T("SAVE %s:nEntry=%d\r\n\r\n"),sLibPath,nEntry );
	}else	{
		ret = fp!=0x0 ? ferror( fp ) : ret;
		G_PRINTF( _T("\t!!!Failed to save %s. err=%d\r\n\r\n"),sLibPath,ret );
	}
...
_FIO_EXIT_:
	...
	if( ret==0 )	{
		...
		G_PRINTF( _T("\t!!!Failed to load %s. err=%d\r\n\r\n"),sLibPath,ret );
	}	
...
//			G_PRINTF( _T("no=%d,w=%g,path=%s\r\n"),no,w_sort[no],hEntry->sFilePath );
			no_e0=-1;
			if( nCls>1 )	{
				d_min=DBL_MAX;			d_max=0.0;
...
//					d = G_fv_distance( hLib->fv_dat,mask,hfv,no,k,temp );
	...
	G_PRINTF( _T("\r\n  total=%d,time=%g,err_1=%g(%d),err_2=%g(%d)\r\n"),nEntry,s,err_1,fail_1,err_2,fail_2 );
