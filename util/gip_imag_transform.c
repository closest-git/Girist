...
void GIP_canny( GIP_OBJECT *hObj,GIP_OBJECT *hBinary,int flag )	{
	int pos,M=hObj->M,N=hObj->N,r,c,step;
	double thresh_1=50.0,thresh_2=100;
	IplImage* hImage,*hEdges;
	char sPath[80];
	uchar* i_data;


	hImage = GIP_set_IplImage_d( hObj->M,hObj->N,hObj->data,hObj->N,0x0 );
	hEdges = cvCloneImage( hImage );
	cvCanny( hImage, hEdges, thresh_1,thresh_2,3 );
	_stprintf( sPath,".\\trace\\canny_%d.jpg",hObj->M*hObj->N );
//	if( !cvSaveImage(sPath,hEdges) ) 		
//		GIP_ERROR( "GIP_canny",-1, "Could not save" );

	step=hEdges->widthStep/sizeof(uchar);
	i_data = (uchar *)hEdges->imageData;
	for( r = 0; r < hEdges->height; r++ )	{
		for( c = 0; c < hEdges->width; c++ )	{
			pos=GIP_RC2POS(r,c,N );	
			hBinary->data[pos] = i_data[r*step+c]==0 ? 1:0;
		}
	}
	GIP_save_IplImage_d( hBinary->M,hBinary->N,sPath,hBinary->data,hBinary->N,0x0 );

GIP_exit:		;
	cvReleaseImage( &hImage );
	cvReleaseImage( &hEdges );
...
void GIP_Histo_Equal(  GIP_OBJECT *hObj,int L,int type,int *temp,int flag )	{
	int i,it,M=hObj->M,N=hObj->N,MN=M*N;
...
//		I_0=MIN(I_0,it);			I_1=MAX(I_1,it);
	...
	if( nz_0==MN )	{
		GIP_ERROR( _T("GIP_Histo_Equal"),-1, _T("Only one level.No contranst here!") );
	}
	s[0]=n[0];
	for( i = 1; i < L; i++ )	{
		s[i]=s[i-1]+n[i];
