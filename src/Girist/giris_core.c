...
//	G_PRINTF( "  {_init_}: M=%d,N=%d,mean=%g,devia=%g,contrast=%g",hObj->M,hObj->N,
...
void  _IRIS_circle_refine_( double *control,G_IRIS_ *hIris,double R_0,double R_1,double r_0,double c_0,int grid,int type )	{
	...
	for( c = 1; c < N-1; c++ )	{
		...
		a = 1.0;
//		a = g_rad[pos]*57.295779513082;
//		a = (22.5<a&&a<=67.5) || (112.5<a&&a<=157.5) || (-22.5>a&&a>=-67.5) || (-112.5>a&&a>=-157.5) ? 1.4 : 1.0;
		if( isDark==1 )
			w[pos] = (a+(I_1-data[pos])*1.0/(I_1-I_0));
		else
			w[pos] = a;
	}
	}
...
//			v = vote[v_pos];
	...
	if( pos_2==-1 )	{
		...
		G_PRINTF( _T("\t!!!Failed:\tNo candidate for sclera!!!"),hIris->s_r,hIris->s_c,hIris->s_radius,hIris->s_match );
	...
	if( hIris->p_match<_ToM_p*1.5 )	{		//INNER_BOUNDARY re-search
		...
		if( hIris->p_match<_ToM_p )	{			
			G_PRINTF( _T("\t!!!Warning:\tpupile(%g,%g,%g),p_match=%4.2g"),hIris->p_r,hIris->p_c,hIris->p_radius,hIris->p_match );
	...
	if( hIris->s_match<_ToM_s*2 || hIris->s_radius<hIris->p_radius )	{				//OUTER_BOUNDARY re-search
		...
		if( hIris->s_match<_ToM_s )	{
			G_PRINTF( _T("\t!!!Warning:\tsclera(%g,%g,%g),s_match=%4.2g"),hIris->s_r,hIris->s_c,hIris->s_radius,hIris->s_match );
			ret = GIRIST_WARN_OUTER_BOUNDARY;		
		}
	}
	w=hIris->p_radius;		
	if( hIris->s_radius<=hIris->p_radius )	{
		G_PRINTF( _T("\t!!!s_radius<=p_radius:\tp_radius=%4.2g,s_radius=%4.2g"),hIris->s_radius,hIris->p_radius );
		ret = GIRIST_ERROR_BOUNDARY;	
	}else if( hIris->p_r-w<0||hIris->p_r+w>=M || hIris->p_c-w<0||hIris->p_c+w>=N )	{
		G_PRINTF( _T("\t!!!Pupil is out of Image:\tp_radius=%4.2g,r=%4.2g,c=%4.2g"),hIris->p_radius,hIris->p_r,hIris->p_c );
		ret = GIRIST_ERROR_BOUNDARY;	
	}
...
//		}		
	...
	if( isReScan==1 )	{
		if( hIris->nSpot>0 )
			hIris->nSpot=_IRIS_spot_remove( control,hIris,hIris->hIa->I_buffer,hIris->hIa->D_buffer,0x0 );
...
void _IRIS_Feature_CYS_( double *control,GIP_OBJECT *hROI,G_FVector_ *hFV,int method,double *D_buffer )	{
	...
	unsigned char *codelets,*V,phase; 
	GIP_FLOATING *data;
	double w,*re,*im,lenda,*feature,*temp,a,b;	
	GIP_GABOR *hGabor;
...
//		GIP_GABOR_filter( hGabor,1,N,data+r*M_step*N,re,im,0x0 );	
		...
		for( c=0; c<N; c++ )	{
			w = atan2( im[c],re[c] );
			feature[GIP_RC2POS( r,c,N )]=w;
//			phase = (0<=w&&w<PI) ? 0x80 : 0x0;
//			feature[GIP_RC2POS( r,c,N )]=phase;
		}
	}
	for( r=0; r<M/M_step; r++ )	{
		for( c=0; c<N; c++ )	{
			...
			a = feature[pos_1];	
			if( 0 )	{
				b=r==0?feature[pos_1+N]:feature[pos_1-N];
			//	b=c==0?feature[pos_1+1]:feature[pos_1-1];
				w = fabs(a-b);
				w = MIN(w, 2*PI-w);
				ASSERT( 0<=w && w<=PI );
				t = w>PI/2 ? 0x80 : 0x0;
...
//	GIP_IMAGE *hIMG_core=hIMG;
	...
	double _ToI_0=control[GIRIS_T_INTN_0],_ToI_1=control[GIRIS_T_INTN_1],q_0=control[GIRIS_SMOOTH_Q],sp;
	clock_t start;
...
//		GIP_OBJ_resize( &obj_0,&(obj_1),5,0 );
		GIP_cv_resize( &obj_0,&(obj_1),0,0 );
//		hIMG_core = GIP_IMG_resize( hIMG,M,N,0x0 );
#ifdef _GIRIST_DEVELOP_
//		G_PRINTF( _T("RESIZE:\tM:%d->%d,N:%d->%d"),hIMG->height,M,hIMG->width,N );	
#endif
	}	else	{
		GIP_OBJ_init( &(obj_1),M,N,0x0 );
...
//		high=(int)(I_0+(I_1-I_0)*(_ToI_1+0.2) );
	...
	w=hIris->p_radius;	
	sp = hIris->nSpot>=1 /*&& hIris->p_radius<15*/ ? 0.5 :1;			//spot会降低p_match
	if( *ret==GIRIST_ERROR_BOUNDARY )	{			//based on circular hash transform
		goto GIP_exit;		
	}else	{
		if( hIris->p_match<control[GIRIS_T_MTCH_P]*sp )	{			
			G_PRINTF( _T("\t!!!Warning:\tpupile(%g,%g,%g),p_match=%4.2g"),hIris->p_r,hIris->p_c,hIris->p_radius,hIris->p_match );
			*ret = GIRIST_WARN_INNER_BOUNDARY;			
		}
...
//	G_PRINTF( _T("%-4d %3g %3g %4.2g %4.2g\t %-5.4g %-5.3g %-5.2g %-4.2g"),1,hIris->p_radius,hIris->s_radius,
...
//		sample=(unsigned char*)GIP_alloc( hfv->unit_size*nSample*N );		
		...
		for( i = 0; i < nSample; i++ )	{
			...
			if( hEntry->cls!=cls_old )	{
				G_PRINTF( _T("%s"),hEntry->sFilePath );
				cls_old = hEntry->cls;
			}
			start=clock( );
...
//			GIP_OBJ_clear( hObj );		GIP_free( hObj );
			...
			G_PRINTF( _T("%-4d %3g %3g %4.2g %4.2g\t %-5.4g %-5.3g %-5.2g %-4.2g"),i+1,hIris->p_radius,hIris->s_radius,
				hIris->p_match,hIris->s_match,hIa->mean,hIa->deviation,hIris->nzNoise*s_N,hIris->x );	
			G_IRIS_clear( hIris );
			GIP_free( hIris );		hIris=GIP_NULL;
			cvReleaseImage( &src_image );
			cvReleaseImage( &img );
//			G_PRINTF( "I_%d:\ttime=%g",i+1,(clock( )-start)*s );	
//			break;
		}
		lib.nSample = nOK;
		G_PRINTF( _T("FINISH:fail=%g(%d),time=%g,Mp_0=%5.3g,Ms_0=%5.3g"),(nSample-nOK)*1.0/nSample,nSample-nOK,(clock( )-start_0)*s,
			Mp_0,Ms_0 );
...
//	unsigned char *sample;
	...
	ASSERT( hLib!=GIP_NULL && hfv!=NULL );
//	GU_SetUserCtrl( hUserCtrl,_T("Begin..."),-1,0x0 );
...
//			GIP_OBJ_clear( hObj );		GIP_free( hObj );
		...
		G_PRINTF( _T("%-4d %3g %3g %4.2g %4.2g\t %-5.4g %-5.3g %-5.2g %-4.2g"),i,hIris->p_radius,hIris->s_radius,
			hIris->p_match,hIris->s_match,hIa->mean,hIa->deviation,hIris->nzNoise*s_N,hIris->x );	
		G_IRIS_clear( hIris );
		GIP_free( hIris );		hIris=GIP_NULL;
		cvReleaseImage( &src_image );
	...
	hLib->nSample = nOK;
	if( i==nSample )	{	//正常结束
		G_PRINTF( _T("FINISH:fail=%g(%d),time=%g,Mp_0=%5.3g,Ms_0=%5.3g"),
			(nSample-nOK)*1.0/nSample,nSample-nOK,(clock( )-start_0)*s,Mp_0,Ms_0 );
		G_PRINTF( _T("F_1=%d,F_2=%d,F_3=%d "),g_nF_1,g_nF_2,g_nF_3 );
	}
...
//	GIP_IMAGE *src_image;
	...
	mod = (int)(control[GIRIS_MODE_]);
	M_normal=(int)(control[GIRIS_M_NORMAL]),		N_normal=(int)(control[GIRIS_N_NORMAL]);
	_ftprintf( fp,_T("mode=%d,M_normal=%d,N_normal=%d"),mod,M_normal,N_normal );
	_ftprintf( fp,_T("\"%s\""),hLib->sPath );
	_ftprintf( fp,_T("nEntry=%d,nClass=%d"),nEntry,hLib->nClass );
	_ftprintf( fp,_T("nOK=%d,nFail=%d"),nSample,nEntry-nSample );

	w_sort=GIP_alloc( sizeof(double)*nSample );
	_ftprintf( fp,_T(" no  p_r p_c R_p s_r s_c R_s ") );
	for( i = 0; i < nEntry; i++ )	{
		hEntry = hLib->arrEntry+i;
		if( hEntry->cls!=cls_old )	{
			_ftprintf( fp, _T("%s"),hEntry->sFilePath );
			cls_old = hEntry->cls;
		}
	//	src_image=GIP_LoadImage( hEntry->sFilePath,iscolor,0x0 );		ASSERT( src_image!=0x0 );
		if( hEntry->status==G_ENTRY_OK )	{
		...
		_ftprintf( fp,_T("%-4d %3g %3g %3g %3g %3g %3g "),i+1,
...
/*
	v0.1	cys
		2/27/2009		
*/
int G_IRIS_identify( GIP_LIB *hLib,G_IRIS_ *hIris,double *dist,int flag )	{
	GU_CTRL_DATA *hUserCtrl=g_hUserCtrl;
	double *control=hLib->user_control,d,d_min,d_max,*w_sort,s,*dist_1;
	G_FVector_ *hfv=hLib->hfv;
	int nShift,i,k,no=-1,oldShift=hfv->nShift,nzPass=0;
	...
	V_len = hfv->V_len;
	_IRIS_Feature_Vector( control,&(hIris->normal),hfv,0x0 );
	V_old=GIP_alloc( hfv->unit_size*V_len );
	GIP_MEMCOPY( V_old,hfv->V,hfv->unit_size*hfv->V_len );
	...
	w_sort=GIP_alloc( sizeof(double)*nEntry*2 );	dist_1=w_sort+nEntry;	
	G_fv_shift_( hfv->type,nShift,V_len,V_old,hfv->V,0x0 );
	G_hamming_dist_( nEntry,dist,hLib->fv_dat,mask,hfv,GIP_NULL,0x0 );	
	for( i = 0; i < nEntry; i++ )		{
		w_sort[i]=0.0;		
		if( arrEntry[i].status!=G_ENTRY_OK )		continue;
		if( dist[i]>0.4 )	{
			dist_1[i]=dist[i];		
			nzPass++;		continue;
		}
		G_fv_shift_( hfv->type,nShift,V_len,hLib->fv_dat+V_len*i,hfv->V,0x0 );
		G_hamming_dist_( 1,dist_1+i,V_old,mask,hfv,GIP_NULL,0x0 );	
	}
	...
	for( i = 0; i < nCls; i++ )	{
		k=C_ptr[i]+1;
		while( k<nEntry && arrEntry[k].cls==i )	k++;
		C_ptr[i+1] = k;
		for( k=C_ptr[i]; k < C_ptr[i+1]; k++ )	{
			if( hUserCtrl!=GIP_NULL && hUserCtrl->bTerminate==TRUE )
				break;
			if( arrEntry[k].status!=G_ENTRY_OK )		continue;
			d = (dist[k]+dist_1[k])/2;			//MAX(dist[k],dist_1[k]);
			dist[k] = d;
			if( d>d_max )		d_max = d;
			if( d<d_min )		
			{	no=k;	d_min = d;	}

	//		G_PRINTF( _T("  I_%d\t%4d %3d %7.3g %7.3g %7.3g(%d) %7.3g\t%5.3g\r\n"),j,i,j-C_ptr[i],
	//			intra_0[j],intra_1[j],inter_0[j],no_e0,inter_1[j],s );
			GU_SetUserCtrl( hUserCtrl,GIP_NULL,k+1,0x0 );
		}
	}
#ifdef _GIRIST_DEVELOP_
...
/*
	返回hEntry_0,hEntry_1的	sym distance

	Copyright 2008-present, Grusoft.
	v0.1	cys
		4/26/2009	
*/
int G_IRIS_distance_2( double *control,G_FVector_ *hfv,GIP_LIB_entry *hEntry_0,GIP_LIB_entry *hEntry_1,double *dist,int flag )	{
	int nEntry=1,V_len,nShift,ret=GIP_OK;
	int mode=(int)(control[GIRIS_MODE_]);
	double dis[2];
	G_FV_TYPE type=hfv->type;
	G_U8 *mask=GIP_NULL,*V_0,*V_1;
	G_IRIS_ *hIris_1,*hIris_0;
	
	V_len = hfv->V_len;
	V_0=GIP_alloc( hfv->unit_size*V_len*2 );			V_1=V_0+hfv->unit_size*V_len;
	hIris_0 = G_IRIS_on_entry( control,hEntry_0,0x0,flag,&ret );
	if( ret!=GIP_OK )	{
		G_IRIS_clear( hIris_0 );			GIP_free( hIris_0 );
		goto QUIT;
	}
	_IRIS_Feature_Vector( control,&(hIris_0->normal),hfv,0x0 );
	GIP_MEMCOPY( V_0,hfv->V,hfv->unit_size*hfv->V_len );
	hIris_1 = G_IRIS_on_entry( control,hEntry_1,0x0,flag,&ret );
	if( ret!=GIP_OK )	{
		G_IRIS_clear( hIris_1 );			GIP_free( hIris_1 );
		goto QUIT;
	}
	_IRIS_Feature_Vector( control,&(hIris_1->normal),hfv,0x0 );
	GIP_MEMCOPY( V_1,hfv->V,hfv->unit_size*hfv->V_len );
...
//	W_nz = W_ptr[W_dim];
	...
	G_fv_shift_( hfv->type,nShift,V_len,V_0,hfv->V,0x0 );
	G_hamming_dist_( nEntry,dis,V_1,mask,hfv,GIP_NULL,0x0 );	
	G_fv_shift_( hfv->type,nShift,V_len,V_1,hfv->V,0x0 );
	G_hamming_dist_( nEntry,dis+1,V_0,mask,hfv,GIP_NULL,0x0 );
	*dist = (dis[0]+dis[1])/2;
