...
*******************缺省配置，适用于MMU*********************		
	...
	3   control[GIRIS_T_INTN_1]=0.6，放大到0.8似乎没差别
		...
		4/10/2009		cys
	1	取消了GIP_lsu_SUPPORTING RANGE检测
		4/11/2009		cys
	1	G_IRIS_process中处理spotduip_match的影响,采用如下公式
		sp = nSpot>1 && hIris->p_radius<15 ? 0.5 :1;

		4/14/2009		cys
	1	control[GIRIS_LG_WAVELEN]从32调高至36
		4/15/2009		cys
	1	control[GIRIS_T_SEP]	取为0.4
		4/16/2009		cys
	1	control[GIRIS_T_SEP]	取为0.36(对应于far=1.0e-4)
...
/*GIRIS_MODE_ */
typedef enum{
	GIRIS_MODE_DEFAULT=0x0,GIRIS_MODE_TRAINING=1,
	GIRIS_MODE_AUTHEN=0x100,GIRIS_MODE_IDENTIFY,
}GIRIST_MODE;
/* 
	global error code
*/
...
struct G_IRIS_tag{
	...
	int shift,type,flag,nzNoise,*mask,nSpot;
