...
/*
	...
	注意：
		1 对于基本矩形公式(3+0)-(1+2)，0,1,2,3四个点必须独立.
		1 基本矩形公式(3+0)-(1+2)的结果，并不包含0-2,0-1两条边

	Copyright 2009-present, Grusoft.
	v0.1	cys
		5/19/2009		
	
	f_rect[1..4]的含义分别为 r,c,h,w,	参见[201] P.23
	pixelsum ( r,c,h,w) = ii(r+h-1,c+w-1)+ ii(r -1,c -1)-ii(r+h-1,c-1)-ii(r-1,c+w-1)
	v0.2	cys
		7/4/2009	
*/
int G_rect2fetr( int *f_rect,int *pos,int *w,int ldx,int flag )	{
	int type,r_0,r_1,c_0,c_1,width,height;
	int s,t,nz=-1,nz_1,i;

	type = f_rect[0];
	height=f_rect[3];				width=f_rect[4];
/*	switch( type )	{
	case G_FETR_1_2:	//4+0-(1+3)-((5+1)-(4+2))
		if( width%2!=0 )
			width+=1;			
		break;
	case G_FETR_2_1: //3+0-(1+2)-((5+2)-(4+3))
		if( height%2!=0 )
			height+=1;
		break;
	case G_FETR_1_3:	//(5+0)-(4+1)-(6+1)+(2+5)+(7+2)-(6+3)
		if( width%3!=0 )	{
			s = (int)ceil(width/3.0);
			width = s*3;
		}
//		s=(c_0*2+c_1)/3;			t=(c_0+c_1*2)/3;	
		break;
	case G_FETR_3_1:	//(3+0)-(2+1)-(5+2)+(3+4)+(4+7)-(5+6)
		if( height%3!=0 )	{
			s = (int)ceil(height/3.0);
			height = s*3;
		}
//		s=(r_0*2+r_1)/3;			t=(r_0+r_1*2)/3;	
		break;
	case G_FETR_2_2:	//(4+0)-(3+1)-(5+1)+(4+2)-(7+3)+(6+4)+(8+4)-(7+5)
		if( width%2!=0 )
			width+=1;
		if( height%2!=0 )
			height+=1;
//		s=(r_0+r_1)/2;			t=(c_0+c_1)/2;	
		break;
	default:
		ASSERT( 0 );
		break;
	}*/
	f_rect[3]=height;			f_rect[4]=width;

	r_0=f_rect[1]-1;			c_0=f_rect[2]-1;
	r_1=f_rect[1]+height-1;		c_1=f_rect[2]+width-1;
	switch( type )	{
	case G_FETR_1_2:	//4+0-(1+3)-((5+1)-(4+2))
		s=(c_0+c_1)/2;
		ASSERT( s>c_0 && s<c_1 && s>=0 );
		pos[0]=GIP_RC2POS( r_0,c_0,ldx );			w[0]=1;
		pos[1]=GIP_RC2POS( r_0,s,ldx );				w[1]=-2;
		pos[2]=GIP_RC2POS( r_0,c_1,ldx );			w[2]=1;
		pos[3]=GIP_RC2POS( r_1,c_0,ldx );			w[3]=-1;
		pos[4]=GIP_RC2POS( r_1,s,ldx );				w[4]=2;
		pos[5]=GIP_RC2POS( r_1,c_1,ldx );			w[5]=-1;
		nz = 6;
		if( r_0<0 )	
		{	w[0]=0;		w[1]=0;		w[2]=0;		}
		if( c_0<0 )	
	...
	case G_FETR_2_1: //3+0-(1+2)-((5+2)-(4+3))
		s=(r_0+r_1)/2;
		ASSERT( s>r_0 && s<r_1 && s>=0 );
		pos[0]=GIP_RC2POS( r_0,c_0,ldx );			w[0]=1;
		pos[1]=GIP_RC2POS( r_0,c_1,ldx );			w[1]=-1;
		pos[2]=GIP_RC2POS( s,c_0,ldx );			w[2]=-2;
		pos[3]=GIP_RC2POS( s,c_1,ldx );			w[3]=2;
		pos[4]=GIP_RC2POS( r_1,c_0,ldx );			w[4]=1;
		pos[5]=GIP_RC2POS( r_1,c_1,ldx );			w[5]=-1;	
		nz = 6;
		if( r_0<0 )	
		{	w[0]=0;		w[1]=0;				}
		...
		{	w[0]=0;		w[2]=0;		w[4]=0;	}
		break;
	case G_FETR_1_3:	//(5+0)-(4+1)-2*(6+1)+2*(2+5)+(7+2)-(6+3)
		s=(c_0*2+c_1)/3;			t=(c_0+c_1*2)/3;	
		ASSERT( s>c_0 && s<t && t<c_1 && s>=0 && t>=0);
		pos[0]=GIP_RC2POS( r_0,c_0,ldx );			w[0]=1;
		pos[1]=GIP_RC2POS( r_0,s,ldx );			w[1]=-3;
		pos[2]=GIP_RC2POS( r_0,t,ldx );			w[2]=3;
		pos[3]=GIP_RC2POS( r_0,c_1,ldx );			w[3]=-1;
		pos[4]=GIP_RC2POS( r_1,c_0,ldx );			w[4]=-1;
		pos[5]=GIP_RC2POS( r_1,s,ldx );			w[5]=3;
		pos[6]=GIP_RC2POS( r_1,t,ldx );			w[6]=-3;
		pos[7]=GIP_RC2POS( r_1,c_1,ldx );			w[7]=1;
		nz = 8;
		if( r_0<0 )	
		...
		{	w[0]=0;		w[4]=0;				}
		break;
	case G_FETR_3_1:	//(3+0)-(2+1)-2*(5+2)+2*(3+4)+(4+7)-(5+6)
		s=(r_0*2+r_1)/3;			t=(r_0+r_1*2)/3;	
		ASSERT( s>r_0 && s<t && t<r_1 && s>=0 && t>= 0 );
		pos[0]=GIP_RC2POS( r_0,c_0,ldx );			w[0]=1;
		pos[1]=GIP_RC2POS( r_0,c_1,ldx );			w[1]=-1;
		pos[2]=GIP_RC2POS( s,c_0,ldx );			w[2]=-3;
		pos[3]=GIP_RC2POS( s,c_1,ldx );			w[3]=3;
		pos[4]=GIP_RC2POS( t,c_0,ldx );			w[4]=3;
		pos[5]=GIP_RC2POS( t,c_1,ldx );			w[5]=-3;
		pos[6]=GIP_RC2POS( r_1,c_0,ldx );			w[6]=-1;
		pos[7]=GIP_RC2POS( r_1,c_1,ldx );			w[7]=1;
		nz = 8;
		if( r_0<0 )	
		{	w[0]=0;		w[1]=0;			}
		if( c_0<0 )	
		{	w[0]=0;		w[2]=0;		w[4]=0;		w[6]=0;		}
		break;
	case G_FETR_2_2:	//(4+0)-(3+1)-(5+1)+(4+2)-(7+3)+(6+4)+(8+4)-(7+5)
		s=(r_0+r_1)/2;			t=(c_0+c_1)/2;	
		ASSERT( s>r_0 && s<r_1 && t>c_0 && t<c_1 && s>=0 && t>=0 );
		pos[0]=GIP_RC2POS( r_0,c_0,ldx );			w[0]=-1;
		pos[1]=GIP_RC2POS( r_0,t,ldx );			w[1]=2;
		pos[2]=GIP_RC2POS( r_0,c_1,ldx );			w[2]=-1;
		pos[3]=GIP_RC2POS( s,c_0,ldx );			w[3]=2;
		pos[4]=GIP_RC2POS( s,t,ldx );				w[4]=-4;
		pos[5]=GIP_RC2POS( s,c_1,ldx );			w[5]=2;
		pos[6]=GIP_RC2POS( r_1,c_0,ldx );			w[6]=-1;
		pos[7]=GIP_RC2POS( r_1,t,ldx );			w[7]=2;
		pos[8]=GIP_RC2POS( r_1,c_1,ldx );			w[8]=-1;
		nz = 9;
		if( r_0<0 )	
		{	w[0]=0;		w[1]=0;		w[2]=0;			}
		if( c_0<0 )	
	...
	for( i = 0; i < nz; i++ )	{
		...
		ASSERT( pos[i]>=0 );
		pos[nz_1] = pos[i];
//		ASSERT( pos[nz_1]>=0 && pos[nz_1]<400 );
		w[nz_1] = w[i]*-1;
		nz_1++;
	}
	nz = nz_1;
...
/*
	...
	v0.1	cys
		9/4/2009		
*/
void _W_rect_set( _W_rect *hRect,float r,float c,float h,float w,float scale )		{
	hRect->r=r;					hRect->c=c;
	hRect->height=h;			hRect->width=w;				
	hRect->scale = scale;
}

/*
	ISSUE-NEEDED:
		1 r_0,r_1,c_0,c_1可对应分数坐标

	Copyright 2009-present, Grusoft.
	v0.1	cys
		9/4/2009		
*/
int W_rect2fetr( _W_rect *w_rect,int nzRect,int *pos,double *w,int ldx,int type )	{
	int i,j,r_0,r_1,c_0,c_1,nz_1=0,nz,h,wd;

	if( type<=G_FETR_POINT )	{
		for( i = 0; i < nzRect; i++ )	{
			r_0=(int)(w_rect[i].r-1);				c_0=(int)(w_rect[i].c-1);
			r_1=(int)(r_0+w_rect[i].height);		c_1=(int)(c_0+w_rect[i].width);
			ASSERT( c_1<ldx );
			if( r_0>=0 && c_0>= 0 )	{
				pos[nz_1] = GIP_RC2POS( r_0,c_0,ldx );
				w[nz_1] = w_rect[i].scale;
				nz_1++;
			}
			if( r_0>=0 )	{
				pos[nz_1] = GIP_RC2POS( r_0,c_1,ldx );
				w[nz_1] = -w_rect[i].scale;
				nz_1++;
			}
			if( c_0>=0 )	{
				pos[nz_1] = GIP_RC2POS( r_1,c_0,ldx );
				w[nz_1] = -w_rect[i].scale;
				nz_1++;
			}		
			pos[nz_1] = GIP_RC2POS( r_1,c_1,ldx );
			w[nz_1] = w_rect[i].scale;
			nz_1++;
		}
	}else	{		//titled
		for( i = 0; i < nzRect; i++ )	{
			r_0=(int)(w_rect[i].r-1);			c_0=(int)(w_rect[i].c-1);
			h=(int)(w_rect[i].height);			wd=(int)(w_rect[i].width);
			ASSERT( c_0+wd<ldx );
			if( r_0>=0 && c_0>= 0 )	{
				pos[nz_1] = GIP_RC2POS( r_0,c_0,ldx );
				w[nz_1] = w_rect[i].scale;
			...
			if( c_0-h>=0 )	{
				pos[nz_1] = GIP_RC2POS( r_0+h,c_0-h,ldx );
				w[nz_1] = -w_rect[i].scale;
				nz_1++;
			}
		//	if( c_0>=0 )	{
				pos[nz_1] = GIP_RC2POS( r_0+wd,c_0+wd,ldx );
				w[nz_1] = -w_rect[i].scale;
				nz_1++;
		//	}		
			pos[nz_1] = GIP_RC2POS( r_0+wd+h,c_0+wd-h,ldx );
			w[nz_1] = w_rect[i].scale;
			nz_1++;
		}	
	}

	for( i = 0; i < nz_1; i++ )	{		//合并相同位置
		if( w[i]==0.0 )
			continue;
