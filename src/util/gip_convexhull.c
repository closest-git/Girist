...
main( int argc, char *argv[] )
{
  ...
  if ( argc > 1 && argv[1][0] == '-' ) {
    ...
    G_PRINTF ("Usage:  %s -d[ebug] c[heck]\n", *argv );
    G_PRINTF ("x y z coords of vertices from stdin\n");
    exit(1);
  }
...
void    LowerFaces( void )
{
   ...
   do {
     ...
     if ( Normz( f ) < 0 ) {
        Flower++;
        f->lower = TRUE;
        /*G_PRINTF("z=%10d; lower face indices: %d, %d, %d\n", z, */
        /*G_PRINTF("lower face indices: %d, %d, %d\n",
           f->vertex[0]->vnum,
           f->vertex[1]->vnum,
           f->vertex[2]->vnum );*/
     }
   ...
   /*G_PRINTF("A total of %d lower faces identified.\n", Flower);*/
}
...
/*
	v0.1	cys
		12/19/2007
*/
void	map_d2i( int n,double *x )	{
	int i,j,*map=(int*)malloc( sizeof(int)*n ),nnz=0;
	double x_step,y_step,*queue=(double*)malloc( sizeof(double)*n ),t,cur;
	...
	cur=queue[0];			x[map[0]]=nnz;
	for( i = 1; i < n; i++ )	{
		while( queue[i]==cur && i < n )		{
			x[map[i]]=nnz;		i++;
		}
		if( i < n )	{
			nnz+=1;
			cur=queue[i];			x[map[i]]=nnz;
		}
	}
	free( queue );
}
...
/*
	...
	v0.1	cys
		12/9/2007
*/
void Delau_Tri_1( int n,double *cord,int *ne,int **e_list )	{
	tVertex  v;
	tFace f;
	tEdge new;
	int i,j,vnum=0,nz=0,*edge=0x0,isValid;
	double *x,*y,xn,yn,zn,dot;

	x=cord;		y=cord+n;
//	map_d2i( n,x );
//	map_d2i( n,y );

//	edge = (int*)malloc( sizeof(int)*n*(n-1) );
	edge = (int*)malloc( sizeof(int)*n*MIN(100,(n-1)) );
	for( i = 0; i < n; i++ )	{
		v = MakeNullVertex();
		v->v[X] = x[i];			v->v[Y] = y[i];
		v->v[Z] = x[i]*x[i]+y[i]*y[i];
		v->vnum = vnum++;
/*		if ( ( abs(x) > SAFE ) || ( abs(y) > SAFE ) || ( abs(z) > SAFE ) ) {
		 G_PRINTF("Coordinate of vertex below might be too large: run with -d flag\n");
		 PrintPoint(v);
		}*/
	}
	DoubleTriangle();
...
//	assert( nz <n*(n-1) ); 
	...
	*e_list = (int*)malloc( sizeof(int)*nz );
	memcpy( *e_list,edge,sizeof(int)*nz );
	*ne = nz;
...
/*
	called by APSYS-regrid

	v0.1	cys
		8/31/2008
*/
int delau_tri_apsys( int n,double *cord,int *edge )	{
	tVertex  v;
	tFace f;
	tEdge new;
	int i,j,vnum=0,nz=0,isValid,ne=0;
	double *x,*y,xn,yn,zn,dot;
...
void	ReadVertices( void )
{
   ...
   while ( scanf ("%d %d", &x, &y ) != EOF )  {
      ...
      v->vnum = vnum++;
      if ( ( abs(x) > SAFE ) || ( abs(y) > SAFE ) || ( abs(z) > SAFE ) ) {
         G_PRINTF("Coordinate of vertex below might be too large: run with -c flag\n");
         PrintPoint(v);
      }
   }
}
...
void	Print( void )
{
   ...
   /* PostScript header */
   G_PRINTF("%%!PS\n");
   G_PRINTF("%%%%BoundingBox: %d %d %d %d\n", 
	  xmin, ymin, xmax, ymax);
   G_PRINTF(".00 .00 setlinewidth\n");
   G_PRINTF("%d %d translate\n", -xmin+100, -ymin+100 );
   ...
   G_PRINTF("\n%%%% Vertices:\tV = %d\n", V);
   G_PRINTF("%%%% index:\tx\ty\tz\n");
   do {                                 
      G_PRINTF( "%%%% %5d:\t%d\t%d\t%d\n", 
	     v->vnum, v->v[X], v->v[Y], v->v[Z] );
      G_PRINTF("newpath\n");
      G_PRINTF("%d\t%d 2 0 360 arc\n", v->v[X], v->v[Y]);
      G_PRINTF("closepath stroke\n\n");
      v = v->next;
   } while ( v != vertices );
   ...
   G_PRINTF("\n%%%% Faces:\tF = %d\n", F );
   G_PRINTF("%%%% Visible faces only: \n");
   do {           
      /* Print face only if it is lower */
      if ( f-> lower )
      {
	 G_PRINTF("%%%% vnums:  %d  %d  %d\n", 
		f->vertex[0]->vnum, 
		f->vertex[1]->vnum, 
		f->vertex[2]->vnum);
	 G_PRINTF("newpath\n");
   ...
   G_PRINTF("%%%% List of all faces: \n");
   G_PRINTF("%%%%\tv0\tv1\tv2\t(vertex indices)\n");
   do {
      G_PRINTF("%%%%\t%d\t%d\t%d\n",
	     f->vertex[0]->vnum,
	     f->vertex[1]->vnum,
...
void    DoubleTriangle( void )
{
   ...
   while ( Collinear( v0, v0->next, v0->next->next ) )
      if ( ( v0 = v0->next ) == vertices )
         G_PRINTF("DoubleTriangle:  All points are Collinear!\n"), exit(0);
   v1 = v0->next;
   v2 = v1->next;
   ...
   while ( !vol )   {
      if ( ( v3 = v3->next ) == v0 ) 
         G_PRINTF("DoubleTriangle:  All points are coplanar!\n"), exit(0);
      vol = VolumeSign( f0, v3 );
   }
...
int  VolumeSign( tFace f, tVertex p )
{
   ...
   if      ( vol > 0.5*DBL_EPSILON )   return  1;
   else if ( vol < -0.5*DBL_EPSILON )  return -1;
   else                    return  0;
}
